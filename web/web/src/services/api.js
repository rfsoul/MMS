...
export const issues = {
  list: async (params = {}) => {
    const qs = new URLSearchParams(params).toString()
    const res = await apiFetch(`/issues${qs ? `?${qs}` : ''}`)
    return res?.issues ?? res ?? []
  },
  ...
  update: async (id, data) => {
    const res = await apiFetch(`/issues/${id}`, { method: 'PATCH', body: JSON.stringify(data) })
    return res?.issue ?? res
  },

  advanceStatus: async (id, status, notes) => {
  ...
  listSymptomCategories: async () => {
    const res = await apiFetch('/issues/symptom-categories')
...
export const reporters = {
  list: async () => {
    const res = await apiFetch('/reporters')
    return res?.reporters ?? res ?? []
  },

  create: async (data) => {
    const res = await apiFetch('/reporters', { method: 'POST', body: JSON.stringify(data) })
    return res?.reporter ?? res
  },
}
