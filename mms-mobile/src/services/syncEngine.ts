      ...
      await db.runAsync(
        `INSERT INTO work_orders
           (id, company_id, title, description, type, status, priority,
            ...
            completed_at, created_at, updated_at, synced_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  ...
  for (const entry of entries) {
    try {
      ...
      if (err instanceof ApiError) {
        // Permanent failures — retrying will never succeed, so remove the
        // entry and repair local state rather than letting it block the outbox.
        const isPermanent =
        ...
        if (isPermanent) {
          console.warn(`[outbox] permanent failure on ${entry.id} (${err.code}) — removing entry`);
          await dbRun(`DELETE FROM outbox WHERE id = ?`, [entry.id]);

          // If a WO completion was rejected, revert local status to in_progress
          // so the technician sees it as still open and can complete tasks first.
          if (entry.entity_type === 'work_order') {
            const payload = JSON.parse(entry.payload);
            if (payload.action === 'complete') {
              const now = new Date().toISOString();
              await dbRun(
                `UPDATE work_orders SET status = 'in_progress', completed_at = NULL, updated_at = ? WHERE id = ?`,
                [now, payload.workOrderId]
              );
              console.warn(`[outbox] reverted WO ${payload.workOrderId} to in_progress`);
            }
          }
        } else {
          await dbRun(
